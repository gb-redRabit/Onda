...
export function createLibraryMenuDefs(deps: LibraryMenuDeps) {
  ...
  function trackMenu(track: MediaFile, opts?: { onEdit?: () => void }) {
    ...
    return { defs, ctx: { track, onEdit: opts?.onEdit } };
  }

  function albumMenu(name: string, tracks: MediaFile[]) {
    ...
    return { defs, ctx: { name, tracks } };
  }

  function folderMenu(folderPath: string, tracks: MediaFile[]) {
    ...
    return { defs, ctx: { folderPath, tracks } };
  }

  function imageMenu(file: MediaFile, onOpen?: () => void) {
    ...
    return { defs, ctx: { file, onOpen } };
  }

  function playlistMenu(playlist: Playlist, onRename: (p: Playlist) => void) {
    ...
    return { defs, ctx: { playlist, onRename } };
  }

  return { trackMenu, albumMenu, folderMenu, imageMenu, playlistMenu };
}
