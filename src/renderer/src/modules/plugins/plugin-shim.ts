...
export const PLUGIN_API_SHIM = [
  ...
  '    try { Object.defineProperty(self, BLOCKED_GLOBALS[bi], { value: undefined, writable: false, configurable: false }); } catch (e) { /* ignore */ }',
  '  }',
  ...
  '    var serialized;',
  '    try { serialized = JSON.stringify(args || []); } catch (e) { serialized = null; }',
  "    if (serialized === null || serialized.length > MAX_ARGS_BYTES) { return Promise.reject(new Error('plugin-op-args-too-large')); }",
  ...
  '    },',
  '    log: {',
  "      info: function (m) { post('log', { level: 'info', message: String(m).slice(0, MAX_LOG_CHARS) }); },",
  "      warn: function (m) { post('log', { level: 'warn', message: String(m).slice(0, MAX_LOG_CHARS) }); },",
  "      error: function (m) { post('log', { level: 'error', message: String(m).slice(0, MAX_LOG_CHARS) }); }",
  '    }',
  '  };',
  '  self.api = api;',
