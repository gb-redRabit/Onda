...
import {
  Minus, Square, X, Search, Music2, Maximize2, FolderOpen, FileAudio
} from '@lucide/vue';
...
const showViewActions = computed(() => ['home', 'library', 'player', 'audio', 'settings'].includes(route.name as string));

async function openFile() {
  const result = (await window.api.invoke('dialog:openFile')) as { filePaths: string[]; canceled: boolean };
  if (result.canceled || !result.filePaths.length) return;
  await openMediaFiles(result.filePaths, router);
}

async function openFolder() {
  const result = (await window.api.invoke('dialog:openFolderFiles')) as { filePaths: string[]; canceled: boolean };
  if (result.canceled || !result.filePaths.length) return;
  await openMediaFiles(result.filePaths, router);
}
...
function minimize() { window.api.invoke('window:minimize'); }
function maximize() { window.api.invoke('window:maximize'); isMaximized.value = !isMaximized.value; }
  ...
  >
    ...
    <div class="flex items-center shrink-0" style="-webkit-app-region: no-drag">
      <div class="flex items-center gap-2 px-3">
        <div class="w-5 h-5 rounded-md flex items-center justify-center bg-linear-to-br from-accent-base to-purple-400">
      ...
      <div class="relative">
        ...
        >
          <button class="w-full px-3 py-1.5 text-left text-xs text-fg-muted hover:bg-accent-ghost hover:text-accent-base transition-colors flex items-center gap-2" @click="openFile(); closeDropdown()">
            <FileAudio :size="13" /> {{ t('menu.openFile') }} <span class="ml-auto text-[10px] text-fg-faint font-mono">Ctrl+O</span>
          </button>
          <button class="w-full px-3 py-1.5 text-left text-xs text-fg-muted hover:bg-accent-ghost hover:text-accent-base transition-colors flex items-center gap-2" @click="openFolder(); closeDropdown()">
            <FolderOpen :size="13" /> {{ t('menu.openFolder') }} <span class="ml-auto text-[10px] text-fg-faint font-mono">Ctrl+Shift+O</span>
          </button>
          <div class="border-t border-border-default my-1 mx-2" />
          <button class="w-full px-3 py-1.5 text-left text-xs text-fg-muted hover:bg-accent-ghost hover:text-accent-base transition-colors" @click="quitApp(); closeDropdown()">
            {{ t('menu.close') }}
          </button>
        </div>
      </div>
    ...
    <div class="flex items-center shrink-0 ml-auto" style="-webkit-app-region: no-drag">
      ...
      <button class="h-9 w-11 flex items-center justify-center hover:bg-bg-hover transition-colors text-fg-muted hover:text-fg-base" @click="minimize">
        <Minus :size="14" />
      </button>
      <button class="h-9 w-11 flex items-center justify-center hover:bg-bg-hover transition-colors text-fg-muted hover:text-fg-base" @click="maximize">
        <Maximize2 v-if="!isMaximized" :size="12" />
        <Square v-else :size="10" />
      </button>
      <button class="h-9 w-11 flex items-center justify-center hover:bg-red-base/80 transition-colors text-fg-muted hover:text-white" @click="closeWin">
        <X :size="14" />
      </button>
    </div>
  </div>
