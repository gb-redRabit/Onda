...
const emit = defineEmits<{
  apply: [data: {
    title?: string; artist?: string; album?: string; year?: number; genre?: string; track?: { no: number };
    coverData?: number[]; coverMime?: string;
  }];
  close: [];
}>();
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <template v-for="rel in releases" :key="rel.id">
            <div class="rounded-xl border border-border-default overflow-hidden">
              <button class="w-full flex items-start gap-3 p-3 hover:bg-bg-hover transition-colors text-left"
                :class="{ 'bg-accent-ghost': selectedId === rel.id }"
                @click="selectRelease(rel)">
                <div class="w-10 h-10 rounded-lg bg-bg-elevated flex items-center justify-center shrink-0 overflow-hidden">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex gap-3 mt-1 text-[11px] text-fg-faint">
                    ...
                    <span class="flex items-center gap-1"><Hash :size="10" />{{ rel.trackCount || rel['track-count'] || '?' }}</span>
                    <span class="flex items-center gap-1"><Album :size="10" />{{ rel.country || '?' }}</span>
                  </div>
                </div>
                <Check v-if="selectedId === rel.id && !lookingUp" :size="16" class="text-accent-base shrink-0 mt-1" />
                <Loader2 v-else-if="lookingUp === rel.id" :size="14" class="animate-spin text-fg-muted shrink-0 mt-1" />
              </button>
