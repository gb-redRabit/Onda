...
api.on('app:start', function () {
  api.settings.get('shape').then(function (shape) {
  ...
  action: function () {
    return api.storage.get('index').then(function (value) {
      ...
      return api.storage.set('index', next).then(function () {
        ...
        return api.visual('element.decoration', { element: 'cover', value: shape.key }).then(function () {
...
api.registerCommand({
  ...
  icon: 'Triangle',
  action: function () {
    return api.visual('element.decoration', { element: 'cover', value: 'triangle' }).then(function () {
...
api.registerCommand({
  ...
  icon: 'Info',
  action: function (ctx) {
    api.log.info('Utwór z menu kontekstowego: ' + ((ctx && ctx.title) || '?') + ' — ' + ((ctx && ctx.path) || '?'));
  }
});